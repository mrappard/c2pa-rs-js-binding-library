...
test('extract signing cert chain', async () => {
  ...
  const result = await signAsset({ format: 'image/jpeg', asset: assetData, manifestDefinition: { claim_generator_info: [{ name: 'test_generator' }], title: 't', assertions: [{ label: 'c2pa.actions', data: { actions: [{ action: 'c2pa.created' }] } }] }, signcert, pkey, alg: 'es256' });
